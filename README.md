...
Set the following environment variables before starting the Gemini CLI. These variables can be loaded from a `.env` file or configured using `gemini extensions config oracledb`.

```bash
export ORACLE_CONNECTION_STRING="<host:port/service_name or TNS alias>"
export ORACLE_USERNAME="<your-oracle-sql-user>"
